...
`InjurySystem::update(Registry&, float dt, float time_scale, bool run_l2)` SHALL run at L1 (60 Hz).
...
`main.cpp` SHALL instantiate `InjurySystem` and call it in the L1 gate (60 Hz):
```cpp
injurySystem.update(registry, dt, time_scale, coordinator.l2_tick_this_frame());
```
`InjurySystem` SHALL run after `BiologySystem` and before `MemoryFormationSystem`.

#### Scenario: InjurySystem runs every frame at L1
- **WHEN** the game loop advances one frame
